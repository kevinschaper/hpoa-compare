...
ancestor/descendant in dismech — a recall gap. Terms are ranked by information
content (rarer/more-specific first); up to 25 per disease are listed.
...
const novel = perDisease.flatMap((d) =>
  d.novel_terms.map((t) => ({disease: d.label, mondo: d.mondo, term: t.id, label: t.label, ic: t.ic})),
);
const missing = perDisease.flatMap((d) =>
  d.missing_terms.map((t) => ({disease: d.label, mondo: d.mondo, term: t.id, label: t.label, ic: t.ic})),
);
```
