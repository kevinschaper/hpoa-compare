...
def micro_average(comparisons: list[DiseaseComparison], attr: str) -> dict:
    """Pool tp/n across diseases for a SetScore attribute ('exact'|'closure')."""
    tp = sum(getattr(c, attr).tp for c in comparisons)
    n_pred = sum(getattr(c, attr).n_pred for c in comparisons)
    n_gold = sum(getattr(c, attr).n_gold for c in comparisons)
...
def macro_average(comparisons: list[DiseaseComparison], attr: str) -> dict:
    ...
    p = sum(getattr(c, attr).precision for c in comparisons) / n
    r = sum(getattr(c, attr).recall for c in comparisons) / n
    f1 = sum(getattr(c, attr).f1 for c in comparisons) / n
    return {"precision": round(p, 4), "recall": round(r, 4), "f1": round(f1, 4)}
